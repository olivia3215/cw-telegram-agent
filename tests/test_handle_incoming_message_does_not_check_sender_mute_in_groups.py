...
async def test_handle_incoming_message_does_not_check_sender_mute_in_groups():
    ...
    with patch("agent_server.incoming.get_channel_name", new=AsyncMock(return_value="x")), patch(
        "agent_server.incoming.mark_partner_typing"
    ), patch("agent_server.incoming.can_agent_send_to_channel", new=AsyncMock(return_value=True)), patch(
