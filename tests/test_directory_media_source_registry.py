# tests/test_directory_media_source_registry.py
#
...
def test_put_serializes_datetime_fields(tmp_path):
    reset_media_source_registry()
    source = get_directory_media_source(tmp_path)
    unique_id = "datetime-field"
    last_used_at = datetime(2025, 1, 2, 3, 4, 5, tzinfo=UTC)
    record = {"unique_id": unique_id, "last_used_at": last_used_at}

    source.put(unique_id, record.copy())

    stored = json.loads((tmp_path / f"{unique_id}.json").read_text(encoding="utf-8"))
    assert stored["last_used_at"] == last_used_at.isoformat()
