...
"""
    ...
    # Add summaries if any exist
    if summaries:
        html_content += '<h2>Conversation Summaries</h2>\n'
                ...
                try:
                    ...
                    if agent_timezone:
                        try:
                            ...
                            created_formatted = dt_local.strftime("%Y-%m-%d %H:%M:%S")
                        except Exception:
                            created_formatted = dt.strftime("%Y-%m-%d %H:%M:%S")
                    else:
                        created_formatted = dt.strftime("%Y-%m-%d %H:%M:%S")
                except Exception:
                    created_formatted = str(created)
            
            html_content += f'''<div class="summary">
    <div class="summary-header">
        <strong>Summary</strong> • Messages {html.escape(str(min_id))} to {html.escape(str(max_id))} • {created_formatted}
    </div>
    <div class="summary-content">{html.escape(summary_text)}</div>
</div>
'''
