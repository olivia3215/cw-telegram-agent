# src/admin_console/agents/conversation_download.py
#
...
def filter_task_logs_for_conversation(messages: list, task_logs: list) -> list:
    """
    Filter task logs to show only successful non-visible tasks within 2 minutes
    before the first message or after.
    
    This is used by the admin console live view.
...
def _interleave_messages_and_logs(messages: list, task_logs: list, summaries: list) -> list:
    ...
    """
    # Filter task logs using the download-specific filter
    logs_to_show = filter_task_logs_for_download(messages, task_logs)
    ...
    # Sort by timestamp
    display_items.sort(key=lambda x: x['timestamp'])
...
"""
    ...
    if show_task_logs:
        display_items = _interleave_messages_and_logs(messages, task_logs, summaries)
    else:
        # Just messages, no interleaving
    ...
    for item in display_items:
        if item['type'] == 'log':
            ...
            html_content += f'''<div class="task-log">
    <div class="task-log-header">
        <strong>[LOG: {html.escape(action_kind.upper())}]</strong>'''
            if task_identifier:
        ...
        # Render message (item['type'] == 'message')
        msg = item['data']
        msg_id = str(msg.get("id", ""))
