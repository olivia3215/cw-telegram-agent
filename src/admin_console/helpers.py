...
async def get_group_participants_map(agent: Agent, group_id: int, entity) -> dict[int, User]:
    """
    Get a mapping of user_id -> User object for all participants in a group/channel.
    
    This is useful for getting participant names when direct get_entity() calls fail
    (e.g., when users aren't in contacts but are group participants).
    
    Args:
        agent: The agent instance
        group_id: The group/channel ID
        entity: The group/channel entity (Chat or Channel)
        
    Returns:
        Dictionary mapping user_id -> User object
    """
    client = agent.client
    participants_map = {}
                    ...
                    for user in participants.users:
                        if isinstance(user, User):
                            ...
                            if getattr(user, "deleted", False):
                                continue
                            participants_map[user.id] = user
            except Exception as e:
                logger.debug(f"[{agent.name}] Error getting participants for channel {group_id}: {e}")
                
        elif isinstance(entity, Chat):
            # For groups, use GetFullChatRequest
            try:
                ...
                if hasattr(full_chat, 'full_chat') and hasattr(full_chat.full_chat, 'participants'):
                    ...
                    if hasattr(participants, 'participants'):
                        # GetFullChatRequest returns participant objects with user_id
                        # We need to fetch the actual User objects
                        user_ids = []
                        ...
                        # Fetch User objects - they should be in the full_chat.users list
                        if hasattr(full_chat, 'users') and full_chat.users:
                            for user in full_chat.users:
                                if isinstance(user, User) and user.id in user_ids:
                                    # Skip deleted users
                                    if getattr(user, "deleted", False):
                                        continue
                                    participants_map[user.id] = user
