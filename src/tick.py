...
async def run_one_tick(work_queue=None, state_file_path: str = None):
        ...
        try:
            ...
            if agent:
                ...
                if agent.is_disabled:
                    logger.info(
                        f"{await format_log_prefix(agent_name, channel_name)} Agent is disabled, cancelling task graph {graph.id}"
                    )
                    work_queue.remove(graph)
                    return
        except Exception as e:
    ...
    logger.info(f"{await format_log_prefix(agent_name, channel_name)} Running task {task.id} of type {task.type}")
    ...
    try:
        ...
        if state_file_path:
            work_queue.save(state_file_path)
        logger.info(f"{await format_log_prefix(agent_name, channel_name)} Task {task.id} is now active.")
        
        handled = await dispatch_task(task.type, task, graph)
        if not handled:
            raise ValueError(f"{await format_log_prefix(agent_name, channel_name)} Unknown task type: {task.type}")
        ...
        if isinstance(e, PeerIdInvalidError) and agent:
            ...
            logger.exception(f"{await format_log_prefix(agent_name, channel_name)} Task {task.id} raised exception: {e}")
    ...
    if is_graph_complete(graph):
        work_queue.remove(graph)
        logger.info(f"{await format_log_prefix(agent_name, channel_name)} Graph {graph.id} completed and removed.")

    # Only save if explicitly requested via parameter, not based on _state_file_path
    # This prevents tests from accidentally writing to the persisted state file
    if state_file_path:
        work_queue.save(state_file_path)
        logger.debug(f"{await format_log_prefix(agent_name, channel_name)} Work queue state saved")
