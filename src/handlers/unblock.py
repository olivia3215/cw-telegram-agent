...
async def handle_unblock(task: TaskNode, graph: TaskGraph, work_queue=None):
    ...
    if entity and is_group_or_channel(entity):
        logger.warning(
            f"{log_prefix} Agent attempted to unblock a group/channel ({channel_id}). Aborting."
        )
        return

    logger.info(f"{log_prefix} Unblocking user {channel_id}.")
    ...
    try:
        ...
        logger.warning(
            f"{log_prefix} Cannot unblock user {channel_id}: {e}. "
