...
async def handle_send_media(task: TaskNode, graph: TaskGraph, work_queue=None):
    ...
    if not unique_id:
        raise ValueError(
            f"{format_log_prefix(agent.name, channel_name)} Send_media task missing 'unique_id' parameter."
        )
    ...
    if not media:
        raise ValueError(
            f"{format_log_prefix(agent.name, channel_name)} Media with unique_id {unique_id!r} not found in cache. "
    ...
    try:
        ...
        logger.exception(
            f"{format_log_prefix(agent.name, channel_name)} Failed to send media: {e}"
        )
