    ...
    *,
    ...
    if not bypass_gagged:
        try:
            ...
            if gagged:
                logger.debug(
                    f"{log_prefix} Skipping received task creation for channel {channel_id} - conversation is gagged"
                )
                return
        except Exception as e:
            logger.warning(f"{log_prefix} Error checking gagged status: {e}")
    ...
    if agent.client is None or not agent.client.is_connected():
        logger.debug(
            f"{log_prefix} Client not connected, attempting to reconnect before creating received task..."
        )
    ...
    async with lock:
        ...
        if old_graph:
                ...
                if task.type == "received" and not task.status.is_completed():
                    ...
                    if reaction_message_id is not None:
                        ...
                        if reaction_message_id in existing_reaction_ids:
                            ...
                            logger.info(
                                f"{log_prefix} Preventing duplicate received task - reaction on message "
                            ...
                            if updated:
                                work_queue.save()
                                logger.debug(f"{log_prefix} Updated flags on existing task {task.id}")
                            return
                        else:
                            # New reaction - will be added to the list below
                            logger.info(
                                f"{log_prefix} Adding reaction on message {reaction_message_id} "
                    ...
                    if clear_reactions:
                        task.params["clear_reactions"] = True
                    logger.debug(
                        f"{log_prefix} Skipping received task creation - active received task "
                    ...
                    try:
                        work_queue.save()
                        logger.debug(f"{log_prefix} Saved work queue state after updating task {task.id}")
                    except Exception as e:
                        logger.error(f"{log_prefix} Failed to save work queue state: {e}")
                    return
            ...
            if received_tasks:
                logger.debug(
                    f"{log_prefix} Found graph for conversation {channel_id} with {len(received_tasks)} "
        ...
        if old_graph:
            ...
            for old_task in old_graph.tasks:
                ...
                if preserve and not old_task.status.is_completed():
                    ...
                    logger.debug(
                        f"{log_prefix} Preserving online wait task {old_task.id} from old graph"
                    )
                    ...
                    logger.debug(
                        f"{log_prefix} Preserving responsiveness delay task {old_task.id} from old graph"
                    )
                else:
        ...
        if agent.is_disabled:
            logger.info(
                f"{log_prefix} Skipping received task creation for disabled agent "
        ...
        if agent.client is None or not agent.client.is_connected():
            logger.debug(
                f"{log_prefix} Client not connected after lock acquisition, "
        ...
        if preserved_online_wait_task:
            ...
            logger.debug(
                f"{log_prefix} Preserved online wait task "
        ...
        if preserved_responsiveness_delay_task:
            ...
            logger.debug(
                f"{log_prefix} Preserved responsiveness delay task "
        ...
        try:
            work_queue.save()
            logger.debug(
                f"{log_prefix} Saved work queue state after inserting task {task_id}"
            )
        except Exception as e:
            logger.error(f"{log_prefix} Failed to save work queue state: {e}")

        logger.info(
            f"{log_prefix} Inserted 'received' task {task_id} "
